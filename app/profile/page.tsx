...
export default function ProfilePage() {
  ...
  const isDirty =
    isComplete &&
    QUIZ_QUESTIONS.some(
      (q) => localPrefs[q.field] !== (profile?.stylePreferences?.[q.field] ?? undefined),
    );
  ...
  const displayInitial =
    (displayName || profile?.displayName || "").charAt(0).toUpperCase() || undefined;

  if (loading) {
    return (
      <section className="app-card shimmer rounded-3xl p-6 text-sm muted-copy">Loading...</section>
    );
  }
    ...
    <form onSubmit={onSave} className="space-y-6">
      ...
      <section className="app-card rounded-3xl p-4">
        <div className="mb-1 flex items-center gap-2">
          ...
          {profile?.hasTryOnPhoto ? <span className="pill pill-success">Uploaded</span> : null}
        </div>

        <p className="mb-1 text-sm muted-copy">
          Upload a clear full-body photo from head to toe. This image will be used for AI outfit
          previews and helps generate more accurate results.
        </p>
        ...
        <div className="space-y-4">
          {tryOnLandscapeWarn ? (
            <p className="text-xs text-foreground">
              This photo appears landscape-oriented. For best results, upload a portrait-oriented
              full-body image.
            </p>
          ) : null}
          ...
          <p className="text-xs muted-copy">
            For best results: stand in a neutral pose with good lighting, visible head to toe. JPG,
